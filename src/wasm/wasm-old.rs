...
pub struct WasmScanner {
    #[cfg(feature = "http")]
    http_scanner: Option<HttpBlockchainScanner<TransactionKeyManagerWrapper<MemoryKeyManagerBackend>>>,
...
impl WasmScanner {
    ...
    pub async fn from_str(data: &str) -> Result<Self, String> {
        // Try view key first
        match Self::from_view_key(data) {
            Ok(scanner) => Ok(scanner),
            Err(view_key_error) => {
                // If view key fails, try seed phrase
                match Self::from_seed_phrase(data).await {
                    Ok(scanner) => Ok(scanner),
    ...
    pub async fn from_seed_phrase(seed_phrase: &str) -> Result<Self, String> {
    ...
    pub fn from_view_key(_view_key_hex: &str) -> Result<Self, String> {
    ...
    pub async fn initialize_http_scanner(&mut self, base_url: &str) -> Result<(), String> {
        if self.http_scanner.is_none() {
            let scanner = HttpBlockchainScanner::new(base_url.to_string(), self.key_manager.clone())
                .await
    ...
    // #[cfg(feature = "http")]
    // fn process_single_http_block_with_scanner(&mut self, http_block: &BlockUtxoInfo) -> Result<(usize, usize),
    // String> { Convert HTTP block to our internal format and process
...
#[wasm_bindgen]
pub async fn create_wasm_scanner(data: &str) -> Result<WasmScanner, JsValue> {
    WasmScanner::from_str(data).await.map_err(|e| JsValue::from_str(&e))
}
...
#[cfg(feature = "http")]
#[wasm_bindgen]
pub async fn scan_blocks(scanner: &mut WasmScanner, start_height: u64, end_height: u64) -> Result<String, JsValue> {
    if let Some(ref mut http_scanner) = scanner.http_scanner {
        let scan_config = http_scanner
        ...
        serde_json::to_string(&blocks)
            .map_err(|e| JsValue::from_str(&format!("Failed to serialize scanned blocks: {}", e)))
    } else {
        Err(JsValue::from_str("HTTP scanner not initialized"))
    }
}
